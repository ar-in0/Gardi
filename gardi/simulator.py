...
class Simulator:
    ...
    def _init_file_upload_callbacks(self):
        ...
        def init_filters(wttContents, wttFilename):
            ...
            try:
                options, station_stats = self.gardi.initialize_parser(wttIO)
                ...
                return (
                    ...
                    [], [], [],
                    [], [], [],
                    html.Div(f"Error parsing WTT: {e}", style={"color": "#dc2626"}),
                )

            n_st = station_stats["stations"]
            n_sp = station_stats["space_stripped"]
            msg = f"{n_st} stations found"
            if n_sp:
                msg += f" ({n_sp} had surrounding spaces — trimmed)"

            return (
                {"initialized": True, "ts": datetime.now().isoformat()},
                options, options, options,
                options, options, options,
                html.Div(msg, style={"color": "#475569", "fontSize": "13px"}),
            )
        ...
        def init_backend(app_state, summaryContents, summaryFilename):
            ...
            try:
                ...
                stats = self.gardi.initialize_backend(summaryIO)
                msg = f"{stats['services']} services found, {stats['rakes']} rake links found"
                return True, html.Div(msg, style={"color": "#475569", "fontSize": "13px"})
            except Exception as e:
                print(f"Error initializing backend: {e}")
                self.gardi.parser = None
                return False, html.Div(f"Error: {e}", style={"color": "#dc2626"})
    ...
    def _init_button_callbacks(self):
        ...
        def on_generate_click(
            ...
            try:
                ...
                if self.gardi.converted_links and self.gardi.query.type == FilterType.RAKELINK:
                    self.gardi.highlight_links(fig, self.gardi.converted_links)
        ...
        def trigger_template_download(n_clicks):
            if not n_clicks:
                raise PreventUpdate
            import zipfile
            buf = io.BytesIO()
            with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
                # Generate a minimal WTT template xlsx
                wtt_buf = io.BytesIO()
                wtt_data = {"STATIONS": ["CHURCHGATE", "MARINE LINES", "CHARNI ROAD", "GRANT ROAD"]}
                with pd.ExcelWriter(wtt_buf, engine="openpyxl") as writer:
                    pd.DataFrame(wtt_data).to_excel(writer, sheet_name="UP", index=False)
                    pd.DataFrame(wtt_data).to_excel(writer, sheet_name="DOWN", index=False)
                zf.writestr("WTT_Template.xlsx", wtt_buf.getvalue())

                # Generate a minimal summary template xlsx
                summary_buf = io.BytesIO()
                summary_data = {
                    "Sr. No": [1], "Link Name": ["A"],
                    "Service 1": ["91001"], "Service 2": ["92001"],
                }
                with pd.ExcelWriter(summary_buf, engine="openpyxl") as writer:
                    pd.DataFrame(summary_data).to_excel(writer, sheet_name="Summary", index=False)
                zf.writestr("WTT_Link_Summary_Template.xlsx", summary_buf.getvalue())

                # Include a short README
                readme = (
                    "Gardi Template Files\n"
                    "====================\n\n"
                    "WTT_Template.xlsx          – Placeholder for the full WTT timetable\n"
                    "WTT_Link_Summary_Template.xlsx – Placeholder for the rake-link summary\n\n"
                    "Replace these with your actual Western Railways WTT and summary files.\n"
                )
                zf.writestr("README.txt", readme)

            buf.seek(0)
            return dcc.send_bytes(buf.getvalue(), "gardi_templates.zip")
