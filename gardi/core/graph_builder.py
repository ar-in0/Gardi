...
def _color(is_ac, alpha=0.8):
    r, g, b = AC_RGB if is_ac else NONAC_RGB
    return f"rgba({r},{g},{b},{alpha})"

def _rgb(is_ac):
    return AC_RGB if is_ac else NONAC_RGB

def _is_dashed(svc):
    """Slow (local) services → dashed line; fast / semi-fast → solid."""
    return svc.line == Line.LOCAL

def _dash_style(dashed):
    return "dash" if dashed else "solid"

# ── switch-point helper ───────────────────────────────────────────────
def _collect_switch_points(svc, stationToY, z_offset, rc_link_name, svc_id_str):
    """Return list of (x, y, z, hover) for stations where fast↔slow changes."""
    pts = []
    if len(svc.lineSwitches) < 2:
        return pts
    switch_stations = {st.upper() for st, _ in svc.lineSwitches}
    for ev in svc.events:
        if ev.isTerminalDeparture or not ev.atTime:
            continue
        stName = str(ev.atStation).strip().upper()
        if stName in switch_stations and stName in stationToY:
            pts.append((
                ev.atTime,
                stationToY[stName],
                z_offset,
                f"⬤ line switch – {rc_link_name}-{svc_id_str}: {stName} "
                f"@ {(int(ev.atTime)//60) % 24:02d}:{int(ev.atTime%60):02d}",
            ))
    return pts
...
class GraphBuilder:
    def build_figure(self, wtt, query, distance_map=None):
        ...
        if is_service_filter:
            # Batch by (is_ac, is_dashed) → 4 possible traces
            batches = {}
            switch_markers = []  # red dots at fast↔slow change points
            ...
            for rc in rakecycles:
                for svc in rc.servicePath:
                    ...
                    key = (svc.needsACRake, _is_dashed(svc))
                    if key not in batches:
                        batches[key] = {"x": [], "y": [], "z": [], "hover": [], "custom": []}
                    batch = batches[key]
                    has_points = False
                    ...
                    # collect switch-point markers
                    switch_markers.extend(
                        _collect_switch_points(svc, stationToY, z_offset, rc.linkName, svc_id_str)
                    )

                    z_labels.append((z_offset, f"{rc.linkName}-{svc_id_str}"))
                    z_offset += 40

            for (is_ac, dashed), b in batches.items():
                if not any(v is not None for v in b["x"]):
                    continue
                color = _color(is_ac)
                ac_label = "AC" if is_ac else "Non-AC"
                spd_label = "Slow" if dashed else "Fast"
                all_traces.append(go.Scatter3d(
                    x=b["x"], y=b["y"], z=b["z"],
                    mode="lines+markers",
                    line=dict(color=color, width=2.5, dash=_dash_style(dashed)),
                    marker=dict(size=2, color=color),
                    hovertext=b["hover"],
                    hoverinfo="text",
                    customdata=b["custom"],
                    name=f"{ac_label} {spd_label}",
                    meta={"ac": is_ac, "dashed": dashed},
                    visible=True,
                ))

            # Switch-point markers (red, slightly thicker)
            if switch_markers:
                all_traces.append(go.Scatter3d(
                    x=[p[0] for p in switch_markers],
                    y=[p[1] for p in switch_markers],
                    z=[p[2] for p in switch_markers],
                    mode="markers",
                    marker=dict(size=5, color="rgba(220,38,38,0.95)", symbol="circle"),
                    hovertext=[p[3] for p in switch_markers],
                    hoverinfo="text",
                    name="Line Switch",
                    showlegend=True,
                ))
            ...
            for rc in rakecycles:
                ...
                # Split events into solid / dashed sub-traces
                sub = {
                    False: {"x": [], "y": [], "z": [], "labels": [], "sids": []},
                    True:  {"x": [], "y": [], "z": [], "labels": [], "sids": []},
                }
                gaps = []  # off-network gap info (shared across sub-traces)
                ...
                for svc in rc.servicePath:
                    ...
                    dashed = _is_dashed(svc)
                    buf = sub[dashed]

                    off_network_buffer = []
                    last_on_network = None
                    svc_has_points = False

                    for ev in svc.events:
                        if not ev.atTime or not ev.atStation or not ev.render:
                            continue
                        ...
                        if off_network_buffer and last_on_network:
                            for lst in (buf["x"], buf["y"], buf["z"], buf["labels"], buf["sids"]):
                                lst.append(None)
                            end_point = (minutes, stationToY[stName], z_offset)
                            gaps.append((last_on_network, end_point, off_network_buffer, svc_id_str))
                            off_network_buffer = []

                        buf["x"].append(minutes)
                        buf["y"].append(stationToY[stName])
                        buf["z"].append(z_offset)
                        buf["labels"].append(stName)
                        buf["sids"].append(svc_id_str)
                        last_on_network = (buf["x"][-1], buf["y"][-1], buf["z"][-1])
                        svc_has_points = True

                    # None gap between services within same sub-trace
                    if svc_has_points:
                        for lst in (buf["x"], buf["y"], buf["z"], buf["labels"], buf["sids"]):
                            lst.append(None)
                ...
                # Emit sub-traces for this rakelink
                for dashed, buf in sub.items():
                    if not any(v is not None for v in buf["x"]):
                        continue
                    spd_tag = " (slow)" if dashed else ""
                    all_traces.append(
                        go.Scatter3d(
                            x=buf["x"],
                            y=buf["y"],
                            ...
                            hovertext=[
                                ...
                                for xx, st, sid in zip(buf["x"], buf["labels"], buf["sids"])
                            ],
                            hoverinfo="text",
                            name=f"{rc.linkName}{spd_tag}",
                            meta={"ac": is_ac, "dashed": dashed},
                            visible=True,
                        )
                    )
                    ...
                    gaps = []  # only emit gaps once (for first sub-trace)
    ...
    def highlight_services(self, fig, selected_services):
        ...
        for trace in fig.data:
            if trace.name == "__focus" or trace.name == "Line Switch":
                continue
            cd = trace.customdata
            if cd is not None and len(cd) > 0:
                ...
                if not selected_set:
                    ...
                    if seg_x:
                        # Preserve dash style from the parent trace
                        dashed = isinstance(trace.meta, dict) and trace.meta.get("dashed", False)
                        fig.add_trace(go.Scatter3d(
                            x=seg_x, y=seg_y, z=seg_z,
                            mode="lines+markers",
                            line=dict(color=bright_color, width=3, dash=_dash_style(dashed)),
    ...
    def highlight_stations(self, fig, station_names):
        ...
        for trace in fig.data:
            if trace.name == "__focus" or trace.name == "Line Switch":
                continue  # preserve focus rings and switch markers
    ...
    def highlight_links(self, fig, selected_links):
        ...
        for trace in fig.data:
            if trace.name == "Line Switch":
                trace.opacity = 0.15 if selected_set else 1.0
                continue
            # Strip " (slow)" suffix and "-sid" suffix for matching
            raw_name = trace.name.split(" (slow)")[0]
            trace_link = raw_name.split("-")[0] if "-" in raw_name else raw_name
