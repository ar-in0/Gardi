...
class Gardi:
    ...
    def initialize_parser(self, wtt_file_obj):
        """Parse WTT, register stations, return station options and stats."""
        self.parser = TimeTableParser()
        self.parser.xlsxToDfFromFileObj(wtt_file_obj)
        station_stats = self.parser.registerStations()

        stations = [s for s in self.parser.wtt.stations]
        options = [{"label": s, "value": s} for s in stations]
        return options, station_stats

    def initialize_backend(self, summary_file_obj):
        """Parse summary, register services, isolate suburban. Returns parse stats dict."""
        ...
        n_services = len(self.parser.wtt.upServices) + len(self.parser.wtt.downServices)
        n_rakes = len(self.parser.wtt.rakecycles)
        return {"services": n_services, "rakes": n_rakes}
